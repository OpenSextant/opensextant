...
public class OpenSextantCorpusProcessor implements CorpusProcessor {
    ...
    private static SimpleDateFormat procdate_fmt = new SimpleDateFormat("yyyyMMMdd_HHmm");

    /** Generates a simple date key for the job */
    public static String getProcessingKey() {
        Date today = new Date();
        return procdate_fmt.format(today);
    }
    ...
    /**
     * The output file name. This is not the full path name; the output directory is provided 
     * separately to the results formatter.
     * 
    ...
    /**
     * The PostConstruct annotation means that this
     * method will be called by Spring once the object has been
    ...
    public String processCorpus(Corpus corpus) throws Exception {
        ...
        /**        
         * Anything else happeningn to the Document, e.g. getting more annotations other than
         * gecoding results must happen before we release the corpus.        
         */
